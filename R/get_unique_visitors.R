#' Returns number of unique visitors by month for a given website
#'
#' The `get_unique_visitors()` function returns the number of unique visitors by month for a given website based on the start and end date you provide.
#'
...
#'
#' @return tibble with date and unique visitor columns
#'
#' @export
#'
#' @examples
#' ##unique_visits <- get_unique_visitors(site = "bbc.com", start = "2020-10", end = "2020-12")

get_unique_visitors <- function(api_key = similarweb_api_key, site, start, end, gr = "monthly", country = "us", md = "False") {
  request = paste0("https://api.similarweb.com/v1/website/", site,
                   "/unique-visitors/desktop_unique_visitors?api_key=", api_key,
                   "&start_date=", start,
                   "&end_date=", end,
                   "&country=", country,
                   "&main_domain_only=", md,
  ...
  if (result$status_code == 200){
    ...
    results <- dplyr::as_tibble(json$unique_visitors)
  }
  results
}

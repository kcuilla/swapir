#' Returns average visit duration by month for a given website
#'
...
#'
#' @return tibble with date and avg visit duration columns
#'
#' @export
#'
#' @examples
...
get_visit_duration <- function(api_key = similarweb_api_key, site, start, end, gr = "monthly", country = "us", md = "False") {
  request = paste0("https://api.similarweb.com/v1/website/", site,
                   "/total-traffic-and-engagement/average-visit-duration?api_key=", api_key,
                   "&start_date=", start,
                   "&end_date=", end,
                   "&country=", country,
                   "&main_domain_only=", md,
  ...
  if (result$status_code == 200){
    ...
    results <- dplyr::as_tibble(json$average_visit_duration)
  }
  results
}

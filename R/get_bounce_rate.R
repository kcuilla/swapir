#' Returns bounce rate by month for a given website
#'
#' The `get_bounce_rate()` function returns the bounce rate by month for a given website based on the start and end date you provide.
#'
...
#'
#' @return tibble with date and bounce rate columns
#'
#' @export
#'
#' @examples
#' ##site_bounce_rate <- get_bounce_rate(site = "bbc.com", start = "2020-10", end = "2020-12")

get_bounce_rate <- function(api_key = similarweb_api_key, site, start, end, gr = "monthly", country = "us", md = "False") {
  request = paste0("https://api.similarweb.com/v1/website/", site,
                   "/total-traffic-and-engagement/bounce-rate?api_key=", api_key,
                   "&start_date=", start,
                   "&end_date=", end,
                   "&country=", country,
                   "&main_domain_only=", md,
  ...
  if (result$status_code == 200){
    ...
    results <- dplyr::as_tibble(json$bounce_rate)
  }
  results
}
